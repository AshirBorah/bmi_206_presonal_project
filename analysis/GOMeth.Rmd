...
pearson_lim <- unname(quantile(permutated_models, 0.99))

median_pearson <- summary %>%
                    group_by(model) %>%
                    summarize(median_pearson=median(pearson))

ggplot(summary, aes(x=pearson, fill=model)) +
...
methylation_models <- summary %>% filter(model=='Methylation') %>%
  pull(pearson)

g <- ggplot(summary, aes(x=pearson))+
  geom_density(aes(fill=model), alpha=0.5)+
  geom_vline(xintercept=median_pearson$Methylation, color='red', linetype='dotted')+
  geom_vline(xintercept=pearson_lim, color='red', linetype='dotted')+
  theme_pubr()

ggsave(here('bmi_206_presonal_project/figures/pearson_distributions.png'), g, width=6, height=8)
